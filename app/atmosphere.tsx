...
export function CitySwitcher({city,setCity,weather}:{city:City;setCity:(c:City)=>void;weather:WeatherState}) {
  ...
  return <div className="environment"><div className="city-switch" aria-label="Site atmosphere location"><button onClick={()=>setCity("SF")} className={city==="SF"?"selected":""} aria-pressed={city==="SF"}><span className="city-glyph sf-glyph" aria-hidden />SF</button><button onClick={()=>setCity("NY")} className={city==="NY"?"selected":""} aria-pressed={city==="NY"}><span className="city-glyph ny-glyph" aria-hidden />NY</button></div><div className="weather-tip" role="status"><strong>{c.name}</strong><span>{weather?`${Math.round(weather.temperature)}° · ${weatherLabel(weather.weatherCode)}`:"Local solar atmosphere"}</span><span>{time}</span></div></div>;
}
