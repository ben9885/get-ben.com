...
export function CitySwitcher({city,setCity,weather,mode,onMode,onPreloadSpace}:{city:City;setCity:(c:City)=>void;weather:WeatherState;mode:EnvironmentMode;onMode:(mode:EnvironmentMode)=>void;onPreloadSpace?:()=>void}) {
  ...
  return <div className={`environment mode-${mode}`}><div className="city-switch" role="group" aria-label="Environmental view"><button onClick={()=>chooseCity("NY")} className={mode==="ny"?"selected":""} aria-pressed={mode==="ny"}><span className="city-glyph city-glyph-ny" aria-hidden />NY</button><button onClick={()=>chooseCity("SF")} className={mode==="sf"?"selected":""} aria-pressed={mode==="sf"}><span className="city-glyph city-glyph-sf" aria-hidden />SF</button><button className={`space-option ${mode==="space"?"selected":""}`} onClick={()=>onMode("space")} onPointerEnter={onPreloadSpace} onFocus={onPreloadSpace} aria-label="Space view" aria-pressed={mode==="space"}><TelescopeIcon />Space</button></div><div className="weather-tip" role="status"><strong>{isSpace?"Space":c.name}</strong><span>{isSpace?"Telescope view":weather?`${Math.round(weather.temperature)}° · ${weatherLabel(weather.weatherCode)}`:"Local solar atmosphere"}</span><span>{time}{isSpace?" UTC":""}</span></div></div>;
}
