  ...
  useEffect(() => {
    ...
    const nebulaMaterial = new THREE.ShaderMaterial({
      uniforms: {
        ...
        warpStrength: { value: 0 }, warpDirection: { value: 1 }, warpCenter: { value: new THREE.Vector2(.5, .5) }
      },
      transparent: true, depthWrite: false, depthTest: false,
      vertexShader: `varying vec2 vUv;void main(){vUv=position.xy*.5+.5;gl_Position=vec4(position.xy,0.,1.);}`,
      fragmentShader: `varying vec2 vUv;uniform float time;uniform vec2 resolution;uniform vec2 pointer;uniform float scroll;uniform float section;uniform vec2 clickPoint;uniform float clickPulse;uniform float warpStrength;uniform float warpDirection;uniform vec2 warpCenter;float hash(vec2 p){p=fract(p*vec2(123.34,345.45));p+=dot(p,p+34.345);return fract(p.x*p.y);}float noise(vec2 p){vec2 i=floor(p),f=fract(p);f=f*f*(3.-2.*f);return mix(mix(hash(i),hash(i+vec2(1.,0.)),f.x),mix(hash(i+vec2(0.,1.)),hash(i+vec2(1.)),f.x),f.y);}float fbm(vec2 p){float value=0.,amplitude=.52;mat2 turn=mat2(.8,.6,-.6,.8);for(int i=0;i<4;i++){value+=noise(p)*amplitude;p=turn*p*2.03+11.7;amplitude*=.5;}return value;}void main(){vec2 uv=vUv;float aspect=resolution.x/max(resolution.y,1.);vec2 p=vec2(uv.x*aspect,uv.y);vec2 focus=vec2(warpCenter.x*aspect,warpCenter.y);p=focus+(p-focus)*(1.+warpStrength*.042);vec2 mouse=vec2(pointer.x*aspect,pointer.y);float drift=time*.018;float sectionWave=sin(section*1.47)*.5+.5;vec2 flow=vec2(drift+scroll*.009+warpDirection*warpStrength*.075,-drift*.42-scroll*.005-warpDirection*warpStrength*.028);float broad=fbm(p*1.08+flow);float detail=fbm(p*2.28-flow*.62+vec2(3.7,8.1));float coolField=smoothstep(.38,.84,broad*.72+detail*.34);float coolAnchor=1.-smoothstep(.12,1.05,distance(p,vec2(aspect*.74,.72)+vec2((mouse.x-aspect*.5)*.06,(mouse.y-.5)*.06)));float cool=coolField*coolAnchor*(.72+sectionWave*.14);float warmNoise=fbm(p*1.18-flow*.73+vec2(9.2,2.6));float warmDetail=fbm(p*2.7+flow*.35+vec2(1.2,6.4));float warmField=smoothstep(.4,.86,warmNoise*.74+warmDetail*.31);float warmAnchor=1.-smoothstep(.08,.82,distance(p,vec2(aspect*.48,-.04)+vec2((mouse.x-aspect*.5)*.035,(mouse.y-.5)*.025)));float warm=warmField*warmAnchor*(.64+(1.-sectionWave)*.14);vec3 voidColor=vec3(.003,.007,.026);vec3 midnight=vec3(.015,.055,.16);vec3 cobalt=vec3(.025,.19,.48);vec3 cyan=vec3(.025,.47,.67);vec3 copper=vec3(.38,.115,.055);vec3 amber=vec3(.78,.38,.105);vec3 violet=vec3(.32,.22,.82);vec3 color=mix(voidColor,midnight,smoothstep(.18,.88,broad)*.44);color+=mix(cobalt,cyan,detail)*cool*.42;color+=mix(copper,amber,warmDetail)*warm*.38;color+=violet*min(cool,warm)*.2;float grain=(hash(gl_FragCoord.xy+time)-.5)*.012*mix(1.,.34,warpStrength);color+=grain;vec2 clickUv=vec2(clickPoint.x*aspect,clickPoint.y);float bloom=exp(-dot(p-clickUv,p-clickUv)*38.)*sin(clickPulse*3.14159);color+=mix(violet,vec3(1.,.68,.3),clickPulse)*bloom*.16;float vignette=smoothstep(1.05,.28,length((uv-.5)*vec2(.92,1.08)));color*=mix(.72+.28*vignette,.5+.5*vignette,clamp(warpStrength,0.,1.));color*=1.+warpStrength*.08;gl_FragColor=vec4(color,.94);}`
    });
    ...
    const starMaterial = new THREE.ShaderMaterial({
      uniforms: {
        ...
        warpStrength: { value: 0 }, warpDirection: { value: 1 }, warpCenter: { value: new THREE.Vector2() }, aspect: { value: innerWidth / innerHeight },
      ...
      vertexShader: `attribute vec3 targetPosition;attribute float size;attribute float depthLayer;attribute float seed;varying vec3 vColor;varying float vSparkle;varying float vHero;varying vec2 vWarpAxis;varying float vWarp;varying float vWarpDirection;varying float vStretch;uniform float morph;uniform float time;uniform vec2 pointer;uniform vec2 pointerPoint;uniform float hoverActive;uniform float scrollForce;uniform float warpStrength;uniform float warpDirection;uniform vec2 warpCenter;uniform float aspect;uniform vec2 gravityPoint;uniform float gravityStrength;uniform vec2 elementPoint;uniform float elementHover;uniform vec2 clickPoint;uniform float pulse;void main(){vColor=color;vSparkle=clamp((size-2.65)*.45,0.,1.);vHero=step(.986,seed);float eased=morph*morph*(3.-2.*morph);vec3 p=mix(position,targetPosition,eased);float factor=depthLayer<.5?.46:depthLayer<1.5?.24:.1;float layerWarp=depthLayer<.5?1.18:depthLayer<1.5?.58:.24;float localWarp=warpStrength*layerWarp;p.xy+=pointer*factor;p.z+=scrollForce*factor*.62;p.xy+=normalize(p.xy+vec2(.0001))*warpDirection*localWarp*.11;vec2 gravity=gravityPoint-p.xy;float gravityInfluence=smoothstep(4.2,.4,length(gravity))*gravityStrength;p.xy+=gravity*gravityInfluence*.008*factor;vec2 hoverDelta=p.xy-pointerPoint;float hoverInfluence=smoothstep(.78,0.,length(hoverDelta))*hoverActive;p.xy+=normalize(hoverDelta+.0001)*hoverInfluence*.034*factor;vec2 elementDelta=p.xy-elementPoint;float elementInfluence=smoothstep(1.35,.18,length(elementDelta))*elementHover;p.xy+=vec2(-elementDelta.y,elementDelta.x)*elementInfluence*.011*factor;float dist=distance(p.xy,clickPoint);float influence=smoothstep(1.45,0.,dist);float compression=pulse<.18?-(pulse/.18):sin((pulse-.18)/.82*3.14159);float wave=exp(-pow((dist-pulse*2.15)*6.,2.))*sin(pulse*12.5664);p.xy+=normalize(p.xy-clickPoint+.0001)*influence*compression*.115;p.z+=wave*.22*(1.-pulse);p.z+=sin(time*.42+seed*18.)*.004*factor;vec4 mv=modelViewMatrix*vec4(p,1.);vec4 clip=projectionMatrix*mv;vec2 centered=vec2((clip.x/max(clip.w,.0001))*aspect,clip.y/max(clip.w,.0001))-vec2(warpCenter.x*aspect,warpCenter.y);vWarpAxis=normalize(centered+vec2(.0001));vWarp=localWarp;vWarpDirection=warpDirection;vStretch=1.+pow(max(localWarp,0.),.78)*(8.8+size*.62);gl_PointSize=min(size*(198./-mv.z)*(1.+wave*.08)*vStretch,72.);gl_Position=clip;}`,
      fragmentShader: `varying vec3 vColor;varying float vSparkle;varying float vHero;varying vec2 vWarpAxis;varying float vWarp;varying float vWarpDirection;varying float vStretch;uniform float opacity;void main(){vec2 q=gl_PointCoord-.5;vec2 axis=normalize(vWarpAxis+vec2(.0001));vec2 normal=vec2(-axis.y,axis.x);float along=dot(q,axis);float across=dot(q,normal);float headOffset=vWarpDirection*vWarp*.2;vec2 headQ=vec2((along-headOffset)*vStretch,across*vStretch);float d=length(headQ);float core=smoothstep(.14,.016,d);float halo=smoothstep(.5,.075,d)*(.12+vHero*vWarp*.1);float cross=(smoothstep(.026,0.,abs(headQ.x))*smoothstep(.38,.07,abs(headQ.y))+smoothstep(.026,0.,abs(headQ.y))*smoothstep(.38,.07,abs(headQ.x)))*vSparkle*(.11+vWarp*.2);float behind=-vWarpDirection*(along-headOffset);float tailLength=.08+vWarp*.58;float tailMask=smoothstep(-.012,.03,behind)*(1.-smoothstep(tailLength*.7,tailLength,behind));float tailWidth=.011+.014*(1.-min(vWarp,1.));float trail=exp(-pow(across/max(tailWidth,.004),2.))*tailMask*vWarp*.52;float blurVelocity=smoothstep(.07,.82,vWarp);float blurWidth=.024+min(vWarp,1.2)*.034;float blurCenter=min(.28,tailLength*.38);float blurSpan=.055+min(vWarp,1.2)*.13;float blurAcross=exp(-pow(across/max(blurWidth,.008),2.));float blurAlong=exp(-pow((behind-blurCenter)/max(blurSpan,.02),2.));float blurGate=smoothstep(-.01,.045,behind)*(1.-smoothstep(.38,.52,behind));float motionBlur=blurAcross*blurAlong*blurGate*blurVelocity*.21;float heroFlare=vHero*vWarp*smoothstep(.018,0.,abs(across))*(1.-smoothstep(.18,.48,abs(along)))*.22;float baseAlpha=(core+halo+cross)*opacity;float trailAlpha=(trail+motionBlur+heroFlare)*opacity;float a=min(1.,baseAlpha+trailAlpha);vec3 sharp=mix(vColor,vec3(1.),core*.62);vec3 directionColor=mix(vec3(1.,.61,.32),vec3(.43,.72,1.),vWarpDirection*.5+.5);vec3 trailColor=mix(vColor,directionColor,.58);vec3 blurColor=mix(trailColor,directionColor,.36);vec3 motionColor=mix(trailColor,blurColor,motionBlur/max(trail+motionBlur,.001));vec3 finalColor=mix(sharp,motionColor,trailAlpha/max(a,.001));gl_FragColor=vec4(finalColor,a);}`
    });
    ...
    const nebulaPointerTarget = new THREE.Vector2(.5, .5), nebulaPointer = new THREE.Vector2(.5, .5), warpFocusTarget = new THREE.Vector2(), warpFocus = new THREE.Vector2();
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      warpFocusTarget.set((event.clientX / innerWidth - .5) * .18, -(event.clientY / innerHeight - .5) * .12);
    ...
    const onPointerLeave = () => { starMaterial.uniforms.hoverActive.value = 0; starMaterial.uniforms.elementHover.value = 0; nebulaPointerTarget.set(.5, .5); warpFocusTarget.set(0, 0); down = false; };
    ...
    let sectionCenters: number[] = [], scrollVelocity = 0, warpStrength = 0, warpDirection = 1, lastScroll = scrollY, lastTime = performance.now(), frame = 0, currentPointerX = 0, currentPointerY = 0, smoothMorph = 0;
    ...
    const animate = (now: number) => {
      ...
      const deltaScroll = scrollY - lastScroll, pixelsPerSecond = deltaScroll / Math.max(dt, .001); lastScroll = scrollY; scrollVelocity += (THREE.MathUtils.clamp(deltaScroll * .0025, -.38, .38) - scrollVelocity) * .1; scrollVelocity *= .88;
      const targetWarp = reduced ? 0 : THREE.MathUtils.smoothstep(Math.abs(pixelsPerSecond), 40, mobile ? 850 : 900) * (mobile ? .62 : 1.12);
      const warpRate = targetWarp > warpStrength ? 12 : 1.65; warpStrength += (targetWarp - warpStrength) * (1 - Math.exp(-warpRate * dt));
      if (Math.abs(pixelsPerSecond) > 12) warpDirection += (Math.sign(pixelsPerSecond) - warpDirection) * (1 - Math.exp(-8 * dt));
      ...
      warpFocus.lerp(warpFocusTarget, reduced ? .18 : .035); starMaterial.uniforms.warpStrength.value = warpStrength; starMaterial.uniforms.warpDirection.value = warpDirection; starMaterial.uniforms.warpCenter.value.copy(warpFocus);
      nebulaPointer.lerp(nebulaPointerTarget, reduced ? .18 : .022); nebulaMaterial.uniforms.pointer.value.copy(nebulaPointer); nebulaMaterial.uniforms.time.value = reduced ? 0 : now * .001; nebulaMaterial.uniforms.scroll.value = reduced ? 0 : scrollY / Math.max(innerHeight, 1); nebulaMaterial.uniforms.section.value = pair + easedMorph; nebulaMaterial.uniforms.warpStrength.value = warpStrength; nebulaMaterial.uniforms.warpDirection.value = warpDirection;
      nebulaMaterial.uniforms.warpCenter.value.set(.5 + warpFocus.x * .5, .5 + warpFocus.y * .5);
      ...
      starMaterial.uniforms.opacity.value += ((activeRef.current ? .88 : 0) - starMaterial.uniforms.opacity.value) * (reduced ? .2 : .04); lineMaterial.opacity += ((activeRef.current ? .035 * linePhase : 0) - lineMaterial.opacity) * .055;
    ...
    const resize = () => { camera.aspect = innerWidth / innerHeight; camera.updateProjectionMatrix(); renderer.setPixelRatio(Math.min(devicePixelRatio, innerWidth < 700 ? 1.2 : 1.5)); renderer.setSize(innerWidth, innerHeight); nebulaMaterial.uniforms.resolution.value.set(innerWidth, innerHeight); starMaterial.uniforms.aspect.value = innerWidth / innerHeight; measure(); };
