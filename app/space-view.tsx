  ...
  useEffect(() => {
    ...
    const starMaterial = new THREE.ShaderMaterial({
      uniforms: {
        ...
        warpStrength: { value: 0 }, warpDirection: { value: 1 }, aspect: { value: innerWidth / innerHeight },
      ...
      vertexShader: `attribute vec3 targetPosition;attribute float size;attribute float depthLayer;attribute float seed;varying vec3 vColor;varying float vSparkle;varying vec2 vWarpAxis;varying float vWarp;varying float vWarpDirection;varying float vStretch;uniform float morph;uniform float time;uniform vec2 pointer;uniform vec2 pointerPoint;uniform float hoverActive;uniform float scrollForce;uniform float warpStrength;uniform float warpDirection;uniform float aspect;uniform vec2 gravityPoint;uniform float gravityStrength;uniform vec2 elementPoint;uniform float elementHover;uniform vec2 clickPoint;uniform float pulse;void main(){vColor=color;vSparkle=clamp((size-2.65)*.45,0.,1.);float eased=morph*morph*(3.-2.*morph);vec3 p=mix(position,targetPosition,eased);float factor=depthLayer<.5?.46:depthLayer<1.5?.24:.1;float layerWarp=depthLayer<.5?1.:depthLayer<1.5?.66:.38;float localWarp=warpStrength*layerWarp;p.xy+=pointer*factor;p.z+=scrollForce*factor*.45;p.xy+=normalize(p.xy+vec2(.0001))*warpDirection*localWarp*.075;vec2 gravity=gravityPoint-p.xy;float gravityInfluence=smoothstep(4.2,.4,length(gravity))*gravityStrength;p.xy+=gravity*gravityInfluence*.008*factor;vec2 hoverDelta=p.xy-pointerPoint;float hoverInfluence=smoothstep(.78,0.,length(hoverDelta))*hoverActive;p.xy+=normalize(hoverDelta+.0001)*hoverInfluence*.034*factor;vec2 elementDelta=p.xy-elementPoint;float elementInfluence=smoothstep(1.35,.18,length(elementDelta))*elementHover;p.xy+=vec2(-elementDelta.y,elementDelta.x)*elementInfluence*.011*factor;float dist=distance(p.xy,clickPoint);float influence=smoothstep(1.45,0.,dist);float compression=pulse<.18?-(pulse/.18):sin((pulse-.18)/.82*3.14159);float wave=exp(-pow((dist-pulse*2.15)*6.,2.))*sin(pulse*12.5664);p.xy+=normalize(p.xy-clickPoint+.0001)*influence*compression*.115;p.z+=wave*.22*(1.-pulse);p.z+=sin(time*.42+seed*18.)*.004*factor;vec4 mv=modelViewMatrix*vec4(p,1.);vec4 clip=projectionMatrix*mv;vec2 centered=vec2((clip.x/max(clip.w,.0001))*aspect,clip.y/max(clip.w,.0001));vWarpAxis=normalize(centered+vec2(.0001));vWarp=localWarp;vWarpDirection=warpDirection;vStretch=1.+localWarp*(5.2+size*.38);gl_PointSize=min(size*(198./-mv.z)*(1.+wave*.08)*vStretch,52.);gl_Position=clip;}`,
      fragmentShader: `varying vec3 vColor;varying float vSparkle;varying vec2 vWarpAxis;varying float vWarp;varying float vWarpDirection;varying float vStretch;uniform float opacity;void main(){vec2 q=gl_PointCoord-.5;vec2 axis=normalize(vWarpAxis+vec2(.0001));vec2 normal=vec2(-axis.y,axis.x);float along=dot(q,axis);float across=dot(q,normal);float headOffset=vWarpDirection*vWarp*.16;vec2 headQ=vec2((along-headOffset)*vStretch,across*vStretch);float d=length(headQ);float core=smoothstep(.135,.018,d);float halo=smoothstep(.5,.09,d)*.12;float cross=(smoothstep(.028,0.,abs(headQ.x))*smoothstep(.34,.08,abs(headQ.y))+smoothstep(.028,0.,abs(headQ.y))*smoothstep(.34,.08,abs(headQ.x)))*vSparkle*.12*(1.-vWarp*.82);float behind=-vWarpDirection*(along-headOffset);float tailLength=.06+vWarp*.38;float tailMask=smoothstep(-.015,.035,behind)*(1.-smoothstep(tailLength*.72,tailLength,behind));float tailWidth=.012+.012*(1.-vWarp);float trail=exp(-pow(across/max(tailWidth,.004),2.))*tailMask*vWarp*.34;float baseAlpha=(core+halo+cross)*opacity;float trailAlpha=trail*opacity;float a=min(1.,baseAlpha+trailAlpha);vec3 sharp=mix(vColor,vec3(1.),core*.58);vec3 trailColor=mix(vColor,vec3(.56,.76,1.),.52);vec3 finalColor=mix(sharp,trailColor,trailAlpha/max(a,.001));gl_FragColor=vec4(finalColor,a);}`
    });
    ...
    const animate = (now: number) => {
      ...
      const targetWarp = reduced ? 0 : THREE.MathUtils.smoothstep(Math.abs(pixelsPerSecond), 100, mobile ? 1150 : 1400) * (mobile ? .55 : 1);
      const warpRate = targetWarp > warpStrength ? 9 : 3.5; warpStrength += (targetWarp - warpStrength) * (1 - Math.exp(-warpRate * dt));
      ...
      starMaterial.uniforms.warpStrength.value = warpStrength; starMaterial.uniforms.warpDirection.value = warpDirection;
